...
### 📊 통계 (확장)
- **종합 통계**: 총 기록일, 완료한 할일, 기록한 생각, 평균 기분
- **시각적 분석**: 
  - 최근 7일 기분 변화 (이모지 포함)
  - 할일 완료율 추이
  - 주간 데일리 리포트 작성 현황
...
- **체계적 회고 시스템**: 
  ...
  - 한 줄 회고 (Weekly summary)
- **주간 네비게이션**: 이전/다음 주 이동 가능
- **편집 모드**: 회고 내용 수정 및 저장

### ⭐ 자기암시 관리 (신규)
- **다중 자기암시 설정**: 여러 개의 자기암시 생성 및 관리
- **활성/비활성 관리**: 필요에 따라 자기암시 활성화/비활성화
- **순서 변경**: 드래그 또는 버튼으로 자기암시 순서 조정
- **편집 기능**: 기존 자기암시 수정 및 삭제
- **작성 가이드**: 효과적인 자기암시 작성을 위한 팁 제공

### 🔧 기술적 기능 (확장)
- **PWA 지원**: 모바일 설치 가능
- **이중 데이터 저장**: Supabase(클라우드) + 로컬스토리지(폴백)
- **자동 마이그레이션**: 로컬스토리지 → Supabase 자동 전환
- **날짜 네비게이션**: 이전/다음 날 이동
- **주간 네비게이션**: 주간 회고 및 통계에서 주별 이동
- **반응형 디자인**: 모바일/데스크톱 최적화
- **실시간 데이터 동기화**: 변경사항 즉시 저장
...
- **UI/UX**: Tailwind CSS v4, Lucide React Icons
- **Drag & Drop**: @dnd-kit (할일 순서 변경, 자기암시 정렬)
- **PWA**: next-pwa
- **데이터베이스**: Supabase (PostgreSQL)
- **폴백 저장소**: 브라우저 로컬스토리지
- **인증**: Supabase Auth (향후 다중 사용자 지원)
...
클라우드 동기화를 원할 경우:
```bash
npm run supabase:setup  # 스키마 확인 및 설정 가이드
```
